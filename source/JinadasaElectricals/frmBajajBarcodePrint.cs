    ...
    public partial class frmBajajBarcodePrint : Form
    {
        ...
        private void textBoxNoOfLabels_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter && textBoxNoOfLabels.Text != string.Empty)
            {
                //Item _item = new Item();
                int noOfLabels=0;
                //if ((int.Parse(textBoxNoOfLabels.Text) % 2) != 0)
                //{
                //    int modAmount = int.Parse(textBoxNoOfLabels.Text) % 2;
                //    noOfLabels= int.Parse(textBoxNoOfLabels.Text) + (2 - modAmount);
                //}
                //else
                //{
                ...
                dgvList.Rows.Add(_item.ItemCode, _item.ItemDescription,_item.PartNo,_item.PackDate, noOfLabels.ToString());
                ...
                foreach (DataGridViewRow  dgvRow in dgvList.Rows)
                {
                    if (dgvRow.Cells["colLabelCount"].Value != null)
                    {
                        ...
                        for (int i = 0; i < labelCount; i++)
                        {
                            ...
                            row["PackDate"] = dgvRow.Cells["colPackDate"].Value.ToString();
        ...
        {
            _foundItems = (from c in dc.Items where c.IsDeleted == false && c.ItemCategory.ItemCategoryDescription.ToUpper() == "BAJAJ" select c).ToList<Item>();
